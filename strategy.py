...
def select_candidates(code: str, name: str, df: pd.DataFrame) -> dict:
    ...
    if (
        pd.isna(ma20_yesterday)
        or pd.isna(ma50_yesterday)
        or pd.isna(ma20_today)
        or pd.isna(ma50_today)
        or pd.isna(rsi_today)
    ):
        return None
    ...
    oversold = (
        rsi_today < 30
    )  # RSI < 30: 과매도 신호 :contentReference[oaicite:30]{index=30}
